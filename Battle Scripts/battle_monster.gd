...
func _init(data: Monster, controller: BattleController = null, p_playerControlled = true) -> void:
	...
	var itemBoosts: Array[int] = heldItem.getBoost(rawData)
...
func getAttack():
	...
	var atk = attack*(1 + attackBonus + temp_attackBonus + atkUpBonus + cardBonus)
...
func getDefense():
	...
	var def = defense*(1 + defenseBonus + defUpBonus)
