...
func toString():
	...
	return '{{tier}|{alignments}|{statWeights}}'.format({'tier':_tier,'alignments':_alignments,'statWeights':_statWeights})
...
static func fromString(itemString: String, cache: MonsterCache):
	...
	return HeldItem.new(_tier, _alignments,_statWeights)

func isNone():
	return (tier == 0) && (len(alignments) == 0) && (Util.abstractSum(statWeights) == 0)

func _init(_tier = 0, _alignment: Array[Card.ALIGNMENT] = [], _statWeights: Array[float] = [0, 0, 0, 0]):
	tier = _tier
