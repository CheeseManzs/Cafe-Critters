...
func updateItems(playerInventory: Inventory):
	...
	for item in locItems:
		# stores each item of the player inventory into the corresponding array
		# each sub-array represents a category that stores items
		# also loads the item ui element
		#print(item)
		var temp = inventoryItem.instantiate()
		temp.initiate(item, locItems[item])
		var tempitem = temp.refItem # stores the item resource being referenced
		inventoryItems[tempitem.tab].append(temp) # tab is an enum property
		# initially only displays the first tab lol
		_on_inventory_tabs_tab_clicked(0)
